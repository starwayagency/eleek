...
function add_true_price() {
  let all_item_price = $('.prod_card-cost');
  let current_valute = $('.scroll_vallute.scroll_changer_active').first().text().replace(/ +/g, ' ').trim();
  all_item_price.map((index, item) => {
    let old_price = $(item).text().replace(/ +/g, ' ').trim();
    let rExp = new RegExp(current_valute, "g");
    let round_price = old_price.replace(rExp, '').replace(',', '.');
    let curr_price = gen_true_price(Math.round(round_price));
    $(item).text(curr_price);
  });
  function gen_true_price(num) {
    let true_price = numberWithSpaces(num);
    let current_valute = $('.scroll_vallute.scroll_changer_active').first().text().replace(/ +/g, ' ').trim();
