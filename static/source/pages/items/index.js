...
function add_true_price() {
  var all_item_price = $('.prod_card-cost');
  var current_valute = $('.scroll_vallute.scroll_changer_active').first().text().replace(/ +/g, ' ').trim();
  all_item_price.map(function (index, item) {
    var old_price = $(item).text().replace(/ +/g, ' ').trim();
    var rExp = new RegExp(current_valute, "g");
    var round_price = old_price.replace(rExp, '').replace(',', '.');
    var curr_price = gen_true_price(Math.round(round_price));
    $(item).text(curr_price);
  });

  function gen_true_price(num) {
    var true_price = numberWithSpaces(num);
    var current_valute = $('.scroll_vallute.scroll_changer_active').first().text().replace(/ +/g, ' ').trim();
    true_price += ".00 ".concat(current_valute);
