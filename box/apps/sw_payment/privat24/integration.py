...
from .models import Privat24Transaction
...
TransactionModel = Privat24Transaction
...
#    test_mode = get_ukrpays_constant("test_mode")
    ...
    def __init__(self, options=None):
        ...
        settings_options = {
            'pay_way': 'privat24',
            'merchant': '146917',
            'password': 'Qs0trEZHPYTGUeqsDSES1Ztd46S6BvIS',
            'ccy': 'UAH', # валюта
            'test_mode': True,
            'return_url':f'{settings.CURRENT_DOMEN}/thank_you/', #страница, принимающая клиента после оплаты
            'server_url':f'{settings.CURRENT_DOMEN}/p24/notify-handler/', #страница, принимающая ответ API о результате платежа
        }
        if not options:
            options = {}
    ...
    def get_urls(self):
        urlpatterns = [
            path('notify-handler/', self.privat24_notify_handler, name="privat24_notify_handler"),
        ]
        return urlpatterns

    def build_signature(self, payment):
        # test_str = f"{payment.encode('utf-8')}{self.password}"
        # return sha1(md5(test_str).hexdigest()).hexdigest()
        payment = payment
        password = self.password
        test_str = f"{payment}{password}".encode('utf-8')
        md = md5(test_str).hexdigest().encode('utf-8')
        x = sha1(md).hexdigest()
        return x

    def handle_request(self, request):
        print(request.POST, '91, integration.py')
        ...
        if local_signature == signature:
            params = dict(parse.parse_qsl(payment))
